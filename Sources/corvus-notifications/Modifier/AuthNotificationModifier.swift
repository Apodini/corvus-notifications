...
/// conforming to `AuthEndpoint`. Requires an object `A` that represents the
/// message to send.
...
AuthEndpoint, RestEndpointModifier where A.QuerySubject: NotificationProtocol {
    ...
    /// The instance of `Endpoint` the `AuthNotificationModifier` is modifying.
...
extension AuthEndpoint where QuerySubject: NotificationProtocol {
    ...
    ///
    /// - Returns: An instance of an `AuthNotificationModifier`
    public func notify() -> AuthNotificationModifier<Self> {
        AuthNotificationModifier(self)
    }
}
